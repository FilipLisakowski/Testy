...
public class MockitoAppTest
{
    ...
    public void sendingConnectionStatus() {
        ...
        ArgumentCaptor<String> getMethod = ArgumentCaptor.forClass(String.class);

        when(msMock.checkConnection(getMethod.capture())).thenReturn(ConnectionStatus.FAILURE);
        assertEquals(1, messenger.testConnection(INVALID_SERVER));
        assertEquals(INVALID_SERVER, getMethod.getValue());
